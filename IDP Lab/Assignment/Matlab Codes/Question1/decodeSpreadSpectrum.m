...
se1 = PSEUDO(seed1,:);
se2 = PSEUDO(seed2,:);
F1 = FREQ(se1(floor(t)+1));
F2 = FREQ(se2(floor(t)+1));
m1 = Y.*cos(2*pi*(F1.*t)); %<-- Write code to demodulate the frequency hopping sequence for seed1

m1 = real(ifft(fft(m1).*LPF)); %Low pass filter (Leave me)
soundsc(m1(1:8:end),fs/8)


m2 = Y.*cos(2*pi*(F2.*t)); %<-- Write code to demodulate the frequency hopping sequence for seed2
...
m = Y.*cos(2*pi*FREQ(19)*t); %<-- Write code to demodulate channel 19 only
...
seed = 354;
se = PSEUDO(seed,:);
F = FREQ(se1(floor(t)+1));
m = Y.*cos(2*pi*(F.*t)); %<-- Write code to demodulate the frequency hopping sequence for seed = 354
...
m1 = Yj.*cos(2*pi*(F1.*t)); %<-- Decode message 1 from the jammed transmission.

m1 = real(ifft(fft(m1).*LPF));
soundsc(m1(1:8:end),fs/8) % ** Turn down your speakers/earphones!! **


m2 = Yj.*cos(2*pi*(F2.*t)); %<-- Decode message 2 from the jammed transmission.
